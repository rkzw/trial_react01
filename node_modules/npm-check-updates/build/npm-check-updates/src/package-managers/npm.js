...
async function viewMany(packageName, fields, currentVersion, options, retried = 0, npmConfigLocal) {
    ...
    try {
        ...
        if (options.retry && ++retried <= options.retry) {
            const packument = await viewMany(packageName, fieldsExtended, currentVersion, options, retried, npmConfigLocal);
            return packument;
        }
        throw err;
    }
    return (0, keyValueBy_1.keyValueBy)(fields, field => {
        const value = field.startsWith('dist-tags.') && result.versions ? result.versions[(0, get_1.default)(result, field)] : result[field];
        return {
            [field]: value
        };
    });
